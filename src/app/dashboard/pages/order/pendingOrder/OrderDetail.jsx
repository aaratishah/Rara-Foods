...
export default function OrderDetail({ detail, isGuest, fetchOrder,noAction }) {
  ...
  return (
    <Row style={{ width: '100%' }} gutter={[16, 16]}>
      {noAction || orderState[detail.status]?.length > 0 && <>
        <Row style={{ width: '100%'}}>
        ...
        <Row style={{marginBottom: 16 }}>
          <Radio.Group>
            {orderState[detail.status].map((currentState) => {
              return <Radio.Button danger={currentState?.type === 'danger'} type={currentState?.type}
      ...
      {detail?.scheduledDate && <Col xs={24} sm={12} md={8}>
        <Card style={{ width: '100%' }}>
          <Card.Meta title="Scheduled At" description={(()=>{
            const isBefore = moment(detail?.scheduledDate).isBefore(moment());
            return isBefore ? moment(detail?.scheduledDate).toNow() : moment(detail?.scheduledDate).fromNow();
          })()}/>
        </Card>
      </Col>}
      {[
